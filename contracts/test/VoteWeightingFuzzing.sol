...
contract VoteWeightingFuzzing {
    ...
    // Dispenser contract (set once at construction; zero address allowed for a general-purpose deployment)
    ...
    /// @param _dispenser Dispenser contract address (zero address for a general-purpose deployment).
    ...
    function _getSum() internal returns (uint256) {
        ...
        for (uint256 i = 0; i < 500; i++) {
            ...
            if (pt.bias > dBias) {
                ...
                // Guarded subtraction so the weekly walk can never underflow-revert
    ...
    function _nomineeRelativeWeight(
        ...
        if (totalSum > 0) {
            ...
            // Cap at 1.0 (1e18) to honor the documented invariant
            if (weight > 1e18) {
                weight = 1e18;
            }
        }
    ...
    function removeNominee(bytes32 account, uint256 chainId) external {
        ...
        // Capture the nominee's currently active aggregate slope before zeroing it
        ...
        // aggregate, guarded for defense in depth
        ...
        // exceeds the maximum four-year veOLAS lock duration.
        uint256 t = nextTime;
        for (uint256 i = 0; i < 500; ++i) {
            t += WEEK;
            uint256 dSlope = changesWeight[nomineeHash][t];
    ...
    function retrieveRemovedNomineeVotingPower(bytes32 account, uint256 chainId) external {
        ...
        // otherwise the removed nominee's slope would be double-subtracted.
