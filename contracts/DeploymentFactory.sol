...
contract DeploymentFactory {
    ...
    function deployVeOLAS(bytes32 salt, address token) external {
        ...
        veOLASAddress = Create2.deploy(0, salt, abi.encodePacked(type(veOLAS).creationCode, abi.encode(token)));
    }
    ...
    function computeVeOLASAddress(bytes32 salt, address token) external view returns (address){
        return Create2.computeAddress(salt, keccak256(abi.encodePacked(type(veOLAS).creationCode, abi.encode(token))));
    }
}