...
contractName="ProcessBridgedDataOptimism"
...
processBridgedDataOptimismAddress=$(echo "$deploymentOutput" | grep 'Deployed to:' | awk '{print $3}')

# Get output length
outputLength=${#processBridgedDataOptimismAddress}
...
echo "$(jq '. += {"processBridgedDataOptimismAddress":"'$processBridgedDataOptimismAddress'"}' $globals)" > $globals

# Verify contract
if [ "$contractVerification" == "true" ]; then
  contractParams="$processBridgedDataOptimismAddress $contractPath"
...
echo "${green}$contractName deployed at: $processBridgedDataOptimismAddress${reset}"
