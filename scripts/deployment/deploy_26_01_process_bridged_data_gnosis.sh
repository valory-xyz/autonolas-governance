...
contractName="ProcessBridgedDataGnosis"
...
processBridgedDataGnosisAddress=$(echo "$deploymentOutput" | grep 'Deployed to:' | awk '{print $3}')

# Get output length
outputLength=${#processBridgedDataGnosisAddress}
...
echo "$(jq '. += {"processBridgedDataGnosisAddress":"'$processBridgedDataGnosisAddress'"}' $globals)" > $globals

# Verify contract
if [ "$contractVerification" == "true" ]; then
  contractParams="$processBridgedDataGnosisAddress $contractPath"
...
echo "${green}$contractName deployed at: $processBridgedDataGnosisAddress${reset}"
