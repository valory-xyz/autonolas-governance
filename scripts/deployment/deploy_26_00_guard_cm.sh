...
CM=$(jq -r '.CM' $globals)
governorAddress=$(jq -r '.governorAddress' $globals)

contractName="GuardCM"
contractPath="contracts/multisigs/$contractName.sol:$contractName"
constructorArgs="$timelockAddress $CM $governorAddress"
...
guardCMAddress=$(echo "$deploymentOutput" | grep 'Deployed to:' | awk '{print $3}')

# Get output length
outputLength=${#guardCMAddress}
...
echo "$(jq '. += {"guardCMAddress":"'$guardCMAddress'"}' $globals)" > $globals

# Verify contract
if [ "$contractVerification" == "true" ]; then
  contractParams="$guardCMAddress $contractPath --constructor-args $(cast abi-encode "constructor(address,address,address)" $constructorArgs)"
...
echo "${green}$contractName deployed at: $guardCMAddress${reset}"
