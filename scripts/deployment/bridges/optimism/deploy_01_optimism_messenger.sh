...
L2CrossDomainMessengerAddress=$(jq -r '.L2CrossDomainMessengerAddress' $globals)
...
contractPath="contracts/bridges/OptimismMessenger.sol:OptimismMessenger"
constructorArgs="$L2CrossDomainMessengerAddress $timelockAddress"
...
optimismMessengerAddress=$(echo "$deploymentOutput" | grep 'Deployed to:' | awk '{print $3}')

# Get output length
outputLength=${#optimismMessengerAddress}
...
echo "$(jq '. += {"optimismMessengerAddress":"'$optimismMessengerAddress'"}' $globals)" > $globals

# Verify contract
if [ "$contractVerification" == "true" ]; then
  contractParams="$optimismMessengerAddress $contractPath --constructor-args $(cast abi-encode "constructor(address,address)" $constructorArgs)"
...
echo "Contract deployed at: $optimismMessengerAddress"