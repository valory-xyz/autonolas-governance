...
initialVotingDelay=$(jq -r '.initialVotingDelay' $globals)
initialVotingPeriod=$(jq -r '.initialVotingPeriod' $globals)
initialProposalThreshold=$(jq -r '.initialProposalThreshold' $globals)
quorum=$(jq -r '.quorum' $globals)
governorDelay=$(jq -r '.governorDelay' $globals)

contractName="GovernorOLAS"
contractPath="contracts/$contractName.sol:$contractName"
constructorArgs="$wveOLASAddress $timelockAddress $initialVotingDelay $initialVotingPeriod $initialProposalThreshold $quorum $governorDelay"
...
governorAddress=$(echo "$deploymentOutput" | grep 'Deployed to:' | awk '{print $3}')

# Get output length
outputLength=${#governorAddress}
...
echo "$(jq '. += {"governorAddress":"'$governorAddress'"}' $globals)" > $globals

# Verify contract
if [ "$contractVerification" == "true" ]; then
  contractParams="$governorAddress $contractPath --constructor-args $(cast abi-encode "constructor(address,address,uint256,uint256,uint256,uint256,uint256)" $constructorArgs)"
...
echo "${green}$contractName deployed at: $governorAddress${reset}"
