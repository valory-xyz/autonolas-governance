...
contractName="ProcessBridgedDataPolygon"
...
processBridgedDataPolygonAddress=$(echo "$deploymentOutput" | grep 'Deployed to:' | awk '{print $3}')

# Get output length
outputLength=${#processBridgedDataPolygonAddress}
...
echo "$(jq '. += {"processBridgedDataPolygonAddress":"'$processBridgedDataPolygonAddress'"}' $globals)" > $globals

# Verify contract
if [ "$contractVerification" == "true" ]; then
  contractParams="$processBridgedDataPolygonAddress $contractPath"
...
echo "${green}$contractName deployed at: $processBridgedDataPolygonAddress${reset}"
